...
//监听RTC相关事件
export default function bindSocketRtc(
  socket: Socket,
  ...
  socket.on(SOCKET_ON_RTC.ANSWER, (data) => {
    ...
    let user = clients.get(toUserName);
    console.log("ANSWER", user, SOCKET_ON_RTC.ANSWER);

    baseEmitHandle(
      data,
      user,
      SOCKET_ON_RTC.ANSWER,
      `转发answer - ${toUserName}不存在`
    );
  });
  ...
  socket.on(SOCKET_ON_RTC.USER_REFUST, (data) => {
    ...
    baseEmitHandle(
      data,
      user,
      SOCKET_ON_RTC.USER_REFUST,
      `转发拒绝 - ${toUserName}不存在`
    );
  });

  function baseEmitHandle(
    data: any,
